...
public class EditPost extends AsyncTask<AddOfferInput, Void, NormalResponse> {

    private final static String URL = UrlConstant.BASE_URL+UrlConstant.EDIT_OFFER_URL;
    private OnLoadCompleteListener callback;
    private Context context;

    public EditPost(Context context, OnLoadCompleteListener cb) {
        callback = cb;
    ...
    private String makeRequest(AddOfferInput addOfferInput) throws Exception {
     ...
     //   nameValuePairs.add(new BasicNameValuePair(NetworkConstant.EMAIL, addOfferInput.getEmail()));
        ...
        nameValuePairs.add(new BasicNameValuePair(NetworkConstant.LATITUDE, addOfferInput.getUserLong()));
        if(addOfferInput.getImage()!=null&&addOfferInput.getImage().size()>0) {
            nameValuePairs.add(new BasicNameValuePair(NetworkConstant.IMAGES, addOfferInput.getImage().toString()));
        }
